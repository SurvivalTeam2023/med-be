...
export default class AudioService {
  ...
  async findAudioById(audioId: number): Promise<Audio> {
    const entity = await this
      ...
      .getOne()
    if (!entity) { ErrorHelper.NotFoundExeption(ERROR_MESSAGE.AUDIO.NOT_FOUND) }

    return entity;

  }
  async findAudios(option: IPaginationOptions, dto: SearchAudioDTO): Promise<Pagination<Audio>> {
