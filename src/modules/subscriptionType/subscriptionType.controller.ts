...
export default class SubscriptionTypeController {
    ...
    async getSubscriptionTypeById(@Param('id') id: number): Promise<SubscriptionTypeEntity> {
    ...
    @Post()
    @Roles({ roles: [USER_CLIENT_ROLE.ADMIN] })
    async createSubscriptionType(@Body() dto: CreateSubscriptionTypeDTO): Promise<SubscriptionTypeEntity> {
    ...
    async updateSubscriptionType(
        ...
        return await this.subscriptionTypeService.updateSubscriptionType(id, updateSubscriptionDTO);
    }
