...
export default class SubscriptionTypeService {
    ...
    async findSubscriptionTypeById(subscriptionTypeId: number): Promise<SubscriptionTypeEntity> {
        const subcription = await this.subscriptionTypeRepo
            .createQueryBuilder('subscription_type')
            .where('subscription_type.id = :subscriptionTypeId', { subscriptionTypeId })
            .getOne();
        if (!subcription) {
    ...
    async createSubscriptionType(dto: CreateSubscriptionTypeDTO): Promise<SubscriptionTypeEntity> {
        const subcription = await this.subscriptionTypeRepo.save({
            ...dto,
        });
        return subcription;
    }
    async updateSubscriptionType(subscriptionTypeId: number, dto: UpdateSubscriptionTypeDTO): Promise<SubscriptionTypeEntity> {
        const subcription = await this.findSubscriptionTypeById(subscriptionTypeId)
        if (!subcription) ErrorHelper.NotFoundExeption(ERROR_MESSAGE.SUBSCRIPTION_TYPE.NOT_FOUND);
    ...
    async deleteSubscriptionType(subscriptionTypeId: number): Promise<SubscriptionTypeEntity> {
        const subsciptionType = await this.findSubscriptionTypeById(subscriptionTypeId)
        if (subsciptionType) {
